...
with DAG(dag_id="joke_DAG", start_date=datetime(2023, 6, 1)):
    say_hello = BashOperator(
        task_id="say_hello", bash_command="echo 'hej hej joking time'"
    )
    ...
    download_norris_joke = BashOperator(
        ...
        bash_command=f"curl -H {data_lake_path}/norris_joke_{time_variable}.json https://api.chucknorris.io/jokes/random",
    )
